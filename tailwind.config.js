module.exports = {
  content: [
    "./pages/*.{html,js}",
    "./index.html",
    "./js/*.js",
    "./components/*.html"
  ],
  theme: {
    extend: {
      colors: {
        // Primary Colors - Trust-building blues
        primary: {
          DEFAULT: "#1E40AF", // blue-800 - Trust-building blue for credibility
          50: "#EFF6FF", // blue-50
          100: "#DBEAFE", // blue-100
          200: "#BFDBFE", // blue-200
          300: "#93C5FD", // blue-300
          400: "#60A5FA", // blue-400
          500: "#3B82F6", // blue-500
          600: "#2563EB", // blue-600
          700: "#1D4ED8", // blue-700
          800: "#1E40AF", // blue-800
          900: "#1E3A8A", // blue-900
        },
        // Secondary Colors - Supporting blues
        secondary: {
          DEFAULT: "#3B82F6", // blue-500 - Supporting blue for headers
          50: "#EFF6FF", // blue-50
          100: "#DBEAFE", // blue-100
          200: "#BFDBFE", // blue-200
          300: "#93C5FD", // blue-300
          400: "#60A5FA", // blue-400
          500: "#3B82F6", // blue-500
          600: "#2563EB", // blue-600
          700: "#1D4ED8", // blue-700
          800: "#1E40AF", // blue-800
          900: "#1E3A8A", // blue-900
        },
        // Accent Colors - High-conversion orange
        accent: {
          DEFAULT: "#EA580C", // orange-600 - High-conversion orange for CTAs
          50: "#FFF7ED", // orange-50
          100: "#FFEDD5", // orange-100
          200: "#FED7AA", // orange-200
          300: "#FDBA74", // orange-300
          400: "#FB923C", // orange-400
          500: "#F97316", // orange-500
          600: "#EA580C", // orange-600
          700: "#C2410C", // orange-700
          800: "#9A3412", // orange-800
          900: "#7C2D12", // orange-900
        },
        // Background Colors
        background: "#FFFFFF", // white - Clean canvas
        surface: "#F8FAFC", // slate-50 - Subtle section separation
        // Text Colors
        text: {
          primary: "#1F2937", // gray-800 - Maximum contrast
          secondary: "#6B7280", // gray-500 - Supporting text
        },
        // Status Colors
        success: {
          DEFAULT: "#059669", // emerald-600 - Guarantee badges
          50: "#ECFDF5", // emerald-50
          100: "#D1FAE5", // emerald-100
          500: "#10B981", // emerald-500
          600: "#059669", // emerald-600
        },
        warning: {
          DEFAULT: "#D97706", // amber-600 - Scarcity timers
          50: "#FFFBEB", // amber-50
          100: "#FEF3C7", // amber-100
          500: "#F59E0B", // amber-500
          600: "#D97706", // amber-600
        },
        error: {
          DEFAULT: "#DC2626", // red-600 - Form validation
          50: "#FEF2F2", // red-50
          100: "#FEE2E2", // red-100
          500: "#EF4444", // red-500
          600: "#DC2626", // red-600
        },
        // Border Colors
        border: {
          DEFAULT: "#E5E7EB", // gray-200 - Minimal borders
          light: "#F3F4F6", // gray-100
        },
      },
      fontFamily: {
        // Headlines: Playfair Display for authority and premium value
        headline: ['Playfair Display', 'serif'],
        playfair: ['Playfair Display', 'serif'],
        // Body: Inter for readability and scanning
        sans: ['Inter', 'sans-serif'],
        inter: ['Inter', 'sans-serif'],
        // Default body font
        body: ['Inter', 'sans-serif'],
      },
      fontSize: {
        // Custom font sizes for landing page hierarchy
        'hero': ['3rem', { lineHeight: '1.1', fontWeight: '700' }],
        'hero-mobile': ['2rem', { lineHeight: '1.1', fontWeight: '700' }],
        'hero-tablet': ['2.5rem', { lineHeight: '1.1', fontWeight: '700' }],
        'section-title': ['2.25rem', { lineHeight: '1.2', fontWeight: '600' }],
        'subsection': ['1.5rem', { lineHeight: '1.3', fontWeight: '600' }],
        'cta': ['1.125rem', { lineHeight: '1.4', fontWeight: '600' }],
        'testimonial': ['1rem', { lineHeight: '1.6', fontWeight: '400' }],
      },
      boxShadow: {
        // Custom shadows for conversion elements
        'cta': '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)',
        'testimonial': '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)',
        'card': '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)',
        'card-hover': '0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)',
      },
      animation: {
        // Custom animations for engagement
        'pulse-cta': 'pulse-cta 2s ease-in-out infinite',
        'fade-in-up': 'fade-in-up 0.6s ease-out',
        'bounce-gentle': 'bounce 1s ease-in-out 2',
      },
      keyframes: {
        'pulse-cta': {
          '0%, 100%': { transform: 'scale(1)' },
          '50%': { transform: 'scale(1.02)' },
        },
        'fade-in-up': {
          'from': {
            opacity: '0',
            transform: 'translateY(20px)',
          },
          'to': {
            opacity: '1',
            transform: 'translateY(0)',
          },
        },
      },
      spacing: {
        // Custom spacing for landing page sections
        '18': '4.5rem',
        '88': '22rem',
        '128': '32rem',
      },
      transitionDuration: {
        // Custom transition durations
        '250': '250ms',
        '400': '400ms',
      },
      transitionTimingFunction: {
        // Custom easing functions
        'in-out-smooth': 'cubic-bezier(0.4, 0, 0.2, 1)',
      },
      borderRadius: {
        // Custom border radius for modern look
        'xl': '0.75rem',
        '2xl': '1rem',
      },
      maxWidth: {
        // Custom max widths for content sections
        'prose': '65ch',
        'content': '1200px',
        'section': '1140px',
      },
      zIndex: {
        // Custom z-index values
        'modal': '1000',
        'dropdown': '100',
        'sticky': '50',
      },
    },
  },
  plugins: [
    // Add any additional plugins here
    function({ addUtilities }) {
      const newUtilities = {
        '.text-balance': {
          'text-wrap': 'balance',
        },
        '.transition-fast': {
          'transition': 'all 250ms ease-in-out',
        },
        '.transition-smooth': {
          'transition': 'all 300ms ease-in-out',
        },
        '.scroll-smooth': {
          'scroll-behavior': 'smooth',
        },
      }
      addUtilities(newUtilities)
    }
  ],
}